...
## Phase 15.1 — Unit + Integration Tests
- `backend/tests/` now has a real, permanent pytest suite (`pip install pytest`, run with `venv\Scripts\python.exe -m pytest tests/` from `backend/`, config in `backend/pytest.ini`). 29 tests: `test_fifo_engine.py` (9 + 1 real-DB integration), `test_tax_classification.py` (5 + 2 real-DB integration), `test_setoff_logic.py` (5), `test_harvesting.py` (7). **29/29 pass** (was 28/29 — see fix below).
- Scope was deliberately FIFO / STCG-LTCG classification / set-off / harvesting only, highest money-risk first — F&O and Crypto engines explicitly NOT tested this phase, per prior project history (built against manual data that will conflict with real broker data).
- Expected values in every test were independently derived BEFORE looking at what the code returns — tax rates/exemption from actual Budget 2024 law (verified via a live web search against Income Tax Department sources, not copied from tax_engine_service.py's constants), the LTCG-loss/STCG-gain set-off rule from Section 70 of the Income Tax Act (same verification), and the two real-DB integration test values hand-computed from the primary test user's actual realized_gains rows before the assertions were written.
- Unit tests use `tests/fakes.py` (in-memory fake repositories, no DB/network — fast). Integration tests either read the documented primary test user's (`765984b3-...`) existing data read-only (via a `db_session` fixture that always rolls back, so `calculate_tax_summary`'s upsert is exercised and verified but never permanently committed — confirmed by direct query afterward: the user's real `tax_summaries` row was untouched), or write through a fully disposable `db_test_user` fixture (fresh `auth.users` + `broker_connection` row per test, deleted in teardown even on failure) — never the shared documented user. Verified zero leftover rows after a full run.
- **Critical finding — real bug, FIXED**: `tax_engine_service.py`'s set-off logic treated STCG-loss-offsets-LTCG-gain and LTCG-loss-offsets-STCG-gain as symmetric mirror cases. They are not. Section 70 only allows a short-term loss to offset either type of gain; a long-term loss may ONLY offset long-term gains, never short-term. Fixed by removing the illegal `elif total_ltcg < 0 and total_stcg > 0` branch entirely — that combination now falls through to the "no set-off" path, so an LTCG loss no longer reduces an STCG gain. `tests/test_setoff_logic.py::test_ltcg_loss_must_not_offset_stcg_gain_section_70` now passes; full suite re-run confirmed no regressions (29/29).
  - **Historical data impact: none.** Checked every `tax_summaries` row in the entire database (not just the test user) against the exact bug-trigger condition (`total_stcg_gains > 0 AND total_ltcg_gains < 0`) — 3 rows exist total, zero match. Went a level deeper and checked every `realized_gains` row in the DB for any negative-LTCG entry at all (the only way the buggy branch could ever have fired) — zero exist, for any user. The bug was real and would have under-taxed STCG the moment it was triggered, but was never actually triggered by any stored data — no historical tax_summaries records need recalculating.
- Documented caveat (not a bug): the 365-day LTCG boundary in `holding_service.py` is a fixed-day-count approximation of "more than 12 months" — true calendar-month arithmetic can be 365 or 366 days depending on which months/leap-years a holding period spans. Boundary tests (364/365/366 days) confirm the fixed-365 rule is applied exactly as documented, not that it's legally identical to calendar-month math in every case.
