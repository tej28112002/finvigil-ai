...
- **Critical finding — real bug, not yet fixed**: `tax_engine_service.py`'s set-off logic treats STCG-loss-offsets-LTCG-gain and LTCG-loss-offsets-STCG-gain as symmetric mirror cases. They are not. Section 70 only allows a short-term loss to offset either type of gain; a long-term loss may ONLY offset long-term gains, never short-term. Right now a user with (say) a real STCG gain and an unrelated LTCG loss gets their STCG under-taxed — the LTCG loss is illegally netted against it instead of carrying forward. `tests/test_setoff_logic.py::test_ltcg_loss_must_not_offset_stcg_gain_section_70` encodes the correct rule and fails against the current implementation on purpose. **Not fixed as part of this phase** — flagged for an explicit decision before CA validation, since it changes real tax-liability figures shown to users/CAs, not an infra config value.
